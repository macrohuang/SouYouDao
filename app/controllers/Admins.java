...
public class Admins extends Application {
  ...
  public static void saveScenic(@Valid Scenic scenic, String roadmapImage, String images,
    ...
    // new ScenicImageDownloader(new String[] {roadmapImage},
    // FileUtil.getApplicationPath("data",
    // "scenic", "roadmaps"), scenic.id,
    // ScenicImageDownloader.SaveType.ROADMAPS).now();
    // }
    // // 保存Scenic Images
    // if (images != null && images.length() > 0) {
    // String[] links = images.replace(" ", "").split("\\r|\\n");
    // // 异步下载图片，存储于/data/scenic/images
    // new ScenicImageDownloader(links, FileUtil.getApplicationPath("data",
    // "scenic", "images"),
    ...
    // scenic.authorizer =
    // Admin.findById(Long.parseLong(session.get(Constants.ADMIN_ID_IN_SESSION)));
    // scenic.save();
    // editScenic(scenic.id, true);
  }

  public static void reIndexScenic() {
    SystemState indexDate =
        SystemState.find("keyName = ? ", SystemState.KEYS.上次景点索引日期.name()).first();
  ...
  /*
   * public static void main(String[] args) throws Exception{ Analyzer analyzer
   * = new IKAnalyzer(); Directory directory = new RAMDirectory(); IndexWriter
   * writer = new
   * IndexWriter(directory,analyzer,true,IndexWriter.MaxFieldLength.LIMITED);
   * Long id = 123l; String title = "石家庄石壁"; String content =
   * "<div><p>&nbsp;&nbsp;&nbsp; 明客家祖地──石壁是客家祖地，现为宁化县西部的一个镇，位于武夷山东麓。古之石壁，土地肥沃、地势平坦、森林茂密，有“玉屏”美称。</p>"
   * +
   ...
   * + "</div>"; Long id2 = 12313l; String title2 = "石家庄"; String content2 =
   * "<div><p>&nbsp;&nbsp;&nbsp; 明客家祖地──石壁是客家祖地，现为宁化县西部的一个镇，位于武夷山东麓。古之石壁，土地肥沃、地势平坦、森林茂密，有“玉屏”美称。</p>"
   * +
   * "<p>　　石壁是客家民系形成的重要地域。许多专家论证，自晋永嘉开始，中原许多汉人为避战乱（经历代五次大迁徙），离开河洛祖地，一路向南流亡，他们翻越武夷山脉，来到宁化石壁繁衍生息，从石壁播衍的客家人地广人多。</p>"
   * +
   ...
   * +
   * "<p>　　“北有大槐树，南有石壁村”。石壁这一客家人的发源地，吸引着许多专家、学者慕名前来采风、调查、考证。随着客家学研究的深入开展，石壁在客家史上的重要地位被进一步确立，众多海内外客家后裔纷来沓至、寻根祭祖、投资开发、观光旅游。</p>"
   * + "</div>"; try{ Document doc1 = new Document(); doc1.add(new Field("id",
   * id+"",Field.Store.YES,Field.Index.NO)); doc1.add(new Field("title",
   * title,Field.Store.YES,Field.Index.ANALYZED)); doc1.add(new Field("content",
   * content,Field.Store.YES,Field.Index.ANALYZED)); Document doc2 = new
   * Document(); doc2.add(new Field("id",
   * id2+"",Field.Store.YES,Field.Index.NO)); doc2.add(new Field("title",
   * title2,Field.Store.YES,Field.Index.ANALYZED)); doc2.add(new
   * Field("content", content2,Field.Store.YES,Field.Index.ANALYZED));
   * writer.addDocument(doc1); writer.addDocument(doc2); writer.close();
   * IndexSearcher searcher = new IndexSearcher(directory); String queryWords =
   * "石壁后裔"; Query query = IKQueryParser.parseMultiField(new
   * String[]{"content","title"},queryWords); searcher.setSimilarity(new
   * IKSimilarity()); TopDocs docs = searcher.search(query, 5); ScoreDoc[]
   * scoreDocs = docs.scoreDocs; for (int i = 0; i < scoreDocs.length; i++) {
